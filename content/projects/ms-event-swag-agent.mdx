...
An events program runs roughly 90 events a quarter, each one needing the right swag for the right attendees, all drawn against a single budget that has to last the whole period. The people requesting swag want to describe an event and get a sensible package back. The ops team needs budget, inventory, allocation rules, and orders in one place instead of scattered across spreadsheets.
...
Second, **personas-as-data.** The early version hardcoded attendee tiers as a TypeScript union—a code change and a deploy every time the program renamed a tier. I tore that out and made personas editable data: rename a tier in settings and it propagates everywhere, no deploy. Boring-sounding, but it's the difference between a tool the ops team owns and a tool they have to file a ticket against.
