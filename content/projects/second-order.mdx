---
title: Second Order
summary: An agent pipeline that produces an independent weekly research briefing end to end. Every number is verified against primary sources, every chart is built from data filed in the repo, and each run records what it learned so the next one is faster.
date: 2026-06
platforms: [Independent research, Agent pipelines, Verified publishing]
...
priority: 30
url: /research/second-order.html
urlLabel: Read the published issues
---
...
Each issue is a PDF report and a companion deck sharing 4 to 10 branded exhibits. Every issue opens with its conclusion and the numbers behind it, then runs numbered evidence sections, a boxed second-order effect, the indicators to watch next, and a source list. Nine issues (one in two parts), a desk sidebar, and a research addendum have shipped.

Read them here: [the published issues of Second Order](/research/second-order.html), each with its companion deck.
