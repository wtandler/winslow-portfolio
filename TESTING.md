...
- **Unit tests** (`test/*.test.ts`): the content loaders in `lib/`. They run against the real `content/` directory and assert structural invariants (required frontmatter, sort order, linked PDFs exist, case study bodies under 1,400 words, no em/en dashes or setup-and-reversal phrases in content bodies), not specific entries — content that satisfies the invariants never breaks them.
